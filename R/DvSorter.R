...
dvSorter <- function(ffDir,
  ...
  nheadrows <- as.numeric(read.csv(dvpath1, fileEncoding='latin1', sep=';', nrows=1, header=FALSE)[,2])
  ...
  dvhead <- read.csv(dvpath1, fileEncoding='latin1', sep=';', nrows=nheadrows, header=FALSE, col.names=sprintf('V%i', 1:ncols), fill=TRUE)
  ...
  pw <- lapply(1:length(dvnames), function(i) {
    ...
    wed <- read.csv(paste0(ffDir, dvnames[i]), fileEncoding='latin1', sep=';', comment.char='"', skip=nheadrows, header=FALSE, col.names=dvcols, fill=TRUE,
                    colClasses=colskips)
  })
  ...
  wellids <- sapply(1:length(dvnames), function(i) {
    ...
    arenaid <- read.csv(paste0(ffDir, dvnames[i]), fileEncoding='latin1', sep=';', skip=wellrow-1, nrows=1, header=FALSE, fill=TRUE)
